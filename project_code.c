...
int main() {
    ...
    if (in_file == NULL) {
        printf("Error al abrir el archivo input.txt");
        return 0;
    }
    
    if (out_file == NULL) {
        printf("Error al abrir el archivo output.txt");
        return 0;
    }
    ...
    while(fgets(line,max_lenght,in_file)!=NULL){
        indice_actual = 0;
        token_anterior = TOKEN_INICIO; 
        fprintf(out_file,"Expresion: %s\n",line);
        fprintf(out_file,"--------------- -------\n");
        fprintf(out_file,"TOKEN\t\tLEXEMA\n");
        fprintf(out_file,"--------------- -------\n");
        
        do{
            t = getNextToken(line, &indice_actual, token_anterior);
